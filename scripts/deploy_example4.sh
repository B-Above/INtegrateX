...
function deploy_contracts() {
   ...
   goduck ether contract deploy --address $ETH_ADDR1 --key-path "$ACCOUNT_PATH"/account.key  --code-path "$CONTRACT_PATH"/blank5.sol >"$CONTRACT_PATH/blank3Addr"
   ...
   print_blue "Deploy L_blank contract"
   goduck ether contract deploy --address $ETH_ADDR1 --key-path "$ACCOUNT_PATH"/account.key  --code-path "$CONTRACT_PATH"/L_blank.sol >"$CONTRACT_PATH/l4Addr"
   l4_address=$(grep Deployed <"$CONTRACT_PATH/l4Addr" | grep -o '0x.\{40\}')
   print_green "L_blank4 contract address: $l4_address"

   print_blue "Deploy blank_chain deep4 contract"
   goduck ether contract deploy --address $ETH_ADDR1 --key-path "$ACCOUNT_PATH"/account.key  --code-path "$CONTRACT_PATH"/blank5_chain.sol >"$CONTRACT_PATH/bcaddr"
   bc_address=$(grep Deployed <"$CONTRACT_PATH/bcaddr" | grep -o '0x.\{40\}')
   print_green "blank0_chain contract address: $bc_address"
...
#    print_green "aduit contract aduit:successful"
  ...
  #  print_green "aduit contract aduit:successful"
   ...
   print_blue "Deploy S_blank contract"
   goduck ether contract deploy --address $ETH_ADDR3 --key-path "$ACCOUNT_PATH"/account.key  --code-path "$CONTRACT_PATH"/S_blank.sol >"$CONTRACT_PATH/S22Addr"
   S22=$(grep Deployed <"$CONTRACT_PATH/S22Addr" | grep -o '0x.\{40\}')
   print_green "S_blank4 contract address: $S22"
   ...
   print_blue "Deploy blank_chain contract"
   goduck ether contract deploy --address $ETH_ADDR3 --key-path "$ACCOUNT_PATH"/account.key  --code-path "$CONTRACT_PATH"/blank_chain.sol >"$CONTRACT_PATH/bc5addr"
   bc5_address=$(grep Deployed <"$CONTRACT_PATH/bc5addr" | grep -o '0x.\{40\}')
   print_green "blank5_chain contract address: $bc5_address"
