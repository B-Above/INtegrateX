...
function deploy_contracts() {
   ...
   goduck ether contract deploy --address $ETH_ADDR1 --key-path "$ACCOUNT_PATH"/account.key  --code-path "$CONTRACT_PATH"/blank3.sol >"$CONTRACT_PATH/blank3Addr"
   ...
   print_blue "Deploy blank3_chain contract"
   goduck ether contract deploy --address $ETH_ADDR1 --key-path "$ACCOUNT_PATH"/account.key  --code-path "$CONTRACT_PATH"/blank3_chain.sol >"$CONTRACT_PATH/bcaddr"
...
#    print_green "aduit contract aduit:successful"
  ...
  #  print_green "aduit contract aduit:successful"
   ...
   goduck ether contract deploy --address $ETH_ADDR2 --key-path "$ACCOUNT_PATH"/account.key  --code-path "$CONTRACT_PATH"/S_blank.sol >"$CONTRACT_PATH/S1Addr"
   S1=$(grep Deployed <"$CONTRACT_PATH/S1Addr" | grep -o '0x.\{40\}')
   print_green "broker_data2 contract address: $S1"
