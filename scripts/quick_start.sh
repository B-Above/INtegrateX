...
BROKERADDR=0x668a209Dc6562707469374B8235e37b8eC25db08
#TRANSFERADDR=0xb00AC45963879cb9118d13120513585873f81Cdb
TRANSFERADDR=0x48c60e86ccb66067F923F3F05ef602Ae2F28cbf9
...
function docker-compose-up() {
  ...
  chmod +x $CURRENT_PATH/trans_start.sh
  ...
  if [ -d $QUICK_PATH_TMP/ethereum3 ]; then
    rm -r $QUICK_PATH_TMP/ethereum3
  fi
  cp -r $ETH_PATH $QUICK_PATH_TMP/ethereum3
  x_replace "s/{{.AppchainAddr}}/ws:\/\/host.docker.internal:8550/" $QUICK_PATH_TMP/ethereum3/ethereum.toml
  x_replace "s/{{.AppchainContractAddr}}/0x857133c5C69e6Ce66F7AD46F200B9B3573e77582/" $QUICK_PATH_TMP/ethereum3/ethereum.toml
  ...
  ETH_PATH_TMP3=$(echo $QUICK_PATH_TMP/ethereum3 | sed 's/\//\\\//g')
  x_replace "s/ethereum-path3/$ETH_PATH_TMP3/g" "${QUICK_PATH_TMP}"/quick_start.yml
  ...
  if [ ! "$(docker network ls -q -f name=quick_start_default)" ]; then
    ...
    if [ "${PROMETHEUS}" == "true" ]; then
      ...
      if [ "${VERSION}" == "v1.23.0" ] ||  [ "${VERSION}" == "v2.8.0" ]; then
        docker-compose -f "${QUICK_PATH_TMP}"/quick_start.yml up -d bitxhub_solo ethereum-1 ethereum-2 ethereum-3
        echo "wait ethereum-1 ethereum-2 ethereum-3 start"
        sleep 5
        bash deploy_contracts.sh deploy http://localhost:8545 http://localhost:8547 http://localhost:8549
        sleep 5
        docker-compose -f "${QUICK_PATH_TMP}"/quick_start.yml up -d pier-ethereum1 pier-ethereum2 pier-ethereum3
      fi
    ...
    if [ "${PROMETHEUS}" == "true" ]; then
      ...
      docker-compose -f "${QUICK_PATH_TMP}"/quick_start.yml restart bitxhub_solo ethereum-1 ethereum-2 ethereum-3 pier-ethereum1 pier-ethereum2 pier-ethereum3
      sleep 5
    fi
  fi

  print_blue "======> init balance"
  ./trans_start.sh
  ...
  pier3CID=$(docker ps -qf name="pier-ethereum3")
  ...
  pier3ID=$(echo $(command_retry "docker exec $pier3CID /root/.pier/scripts/getPierId.sh") | awk -F " " '{print $NF}')
  print_blue "======> pier1ID：$pier1ID,pier2ID:$pier2ID,pier3ID:$pier3ID"
  ...
  if [ "${VERSION}" == "v1.11.0" ]; then
    ...
    # command_retry "docker exec $bitxhubCID bitxhub client  transfer --key /root/.bitxhub/key.json --to 0xddbAbD3197DCcAF871C7D7FDd87aC7DC292f46A5 --amount 100000000000000000"
    command_retry "docker exec $bitxhubCID bitxhub client  transfer --key /root/.bitxhub/key.json --to $pier1ID --amount 100000000000000000"
    command_retry "docker exec $bitxhubCID bitxhub client  transfer --key /root/.bitxhub/key.json --to $pier2ID --amount 100000000000000000"
    command_retry "docker exec $bitxhubCID bitxhub client  transfer --key /root/.bitxhub/key.json --to $pier3ID --amount 100000000000000000"
  fi
  ...
  if [ "${VERSION}" == "v1.11.3" ]; then
    ...
    command_retry "docker exec $pier3CID /root/.pier/scripts/registerAppchain.sh ethappchain3 chainC ethereum chainC-description 1.9.13 /root/.pier/ethereum/ether.validators consensusType "${VERSION}" "${BROKERADDR}" "${pier3CID}" "
  fi
  ...
  proposal31ID="${pier3ID}-0"
  command_retry "docker exec $bitxhubCID /root/.bitxhub/scripts/vote.sh $proposal31ID approve reason"
  ...
  if [ "${VERSION}" == "v1.23.0" ] || [ "${VERSION}" == "v2.8.0" ]; then
      ...
      command_retry "docker exec $pier3CID /root/.pier/scripts/registerService.sh ethappchain3 "${TRANSFERADDR}" eth3 "${VERSION}""
      ...
      proposal32ID="${pier3ID}-1"
      command_retry "docker exec $bitxhubCID /root/.bitxhub/scripts/vote.sh $proposal32ID approve reason"
  fi
  ...
  if [ "${PROMETHEUS}" == "true" ]; then
    docker-compose -f "$QUICK_PATH_TMP"/quick_start.yml logs --follow bitxhub_solo ethereum-1 ethereum-2 ethereum-3 pier-ethereum1 pier-ethereum2  pier-ethereum3 prom grafana
  else
    docker-compose -f "$QUICK_PATH_TMP"/quick_start.yml logs --follow bitxhub_solo ethereum-1 ethereum-2 ethereum-3 pier-ethereum1 pier-ethereum2  pier-ethereum3
  fi
...
function queryAccount() {
  ...
  goduck ether contract invoke \
    ...
    --abi-path=./pier/ethereum/$1/transfer.abi  0x668a209Dc6562707469374B8235e37b8eC25db08 getBalance Alice
  print_blue "Query Alice account in ethereum-3 appchain"
  goduck ether contract invoke \
    --key-path ./docker/quick_start/account.key --address http://localhost:8549 \
    --abi-path=./pier/ethereum/$1/transfer.abi  0x668a209Dc6562707469374B8235e37b8eC25db08 getBalance Alice
}
...
function interchainTransfer() {
  ...
  if [ "$2" == "v1.23.0"  ] || [ "$2" == "v2.8.0"  ]; then
      ...
      goduck ether contract invoke \
              --key-path ./docker/quick_start/account.key --abi-path ./example/transfer.abi \
              --address http://localhost:8545 \
               0xb00AC45963879cb9118d13120513585873f81Cdb transfer 1356:ethappchain3:0xb00AC45963879cb9118d13120513585873f81Cdb^Alice^Alice^1000

      print_blue "5. Query accounts after the first-round invocation"
      queryAccountNew

      print_blue "7. Send 500 coin from Alice in ethereum-3 to Alice in ethereum-2"
      goduck ether contract invoke \
                    --key-path ./docker/quick_start/account.key --abi-path ./example/transfer.abi \
                    --address http://localhost:8549 \
                     0xb00AC45963879cb9118d13120513585873f81Cdb transfer 1356:ethappchain2:0xb00AC45963879cb9118d13120513585873f81Cdb^Alice^Alice^500
